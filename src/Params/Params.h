...
public:
    ...
    //! Do we solve poisson
    bool solve_poisson;
    //! Do we solve dielectrics
    bool with_epsilon;
    //! Including nSurf;
    bool with_nSurf;
    //! defines the type of nSurf ("specular" or "diffuse")
    std::string nSurf_type;
    double specularity;
    //! Do we solve in quasistatic regime (without Faraday eqn)
    bool quasistatic;
    //! only constant PrescribedField?
    bool constantPrescribedFieldsOnly;
    //! Do we solve mfp
    bool with_mfp;
    ...
    //Ex_const to add to field, not participating in Maxwell's solver
    // double Ex_const;
