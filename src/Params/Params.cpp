    ...
    namelist( "" )
{
    ...
    if( PyTools::nComponents( "Checkpoints" )>0 && PyTools::extractV( "restart_files", _unused_restart_files, "Checkpoints" ) ) {
        int restart_mode;
        PyTools::extract( "restart_mode", restart_mode, "Checkpoints" );

        if(restart_mode==0 ){
            MESSAGE( 1, "Code will restart without reapplying external fields" );
            restart=true;
        }
        else if(restart_mode==1){
            MESSAGE( 1, "Code will restart with reapplying external fields" );
            restart_add_field=true;
            restart=true;
        }
    }
    ...
    PyTools::extract( "with_epsilon", with_epsilon, "Main"   );
    // nSurf condition
    PyTools::extract( "with_nSurf", with_nSurf, "Main"   );
    // nSurf type ("specular" of "diffuse")
    PyTools::extract( "nSurf_type", nSurf_type, "Main"   );
    if( nSurf_type!="specular"
                && nSurf_type!="diffuse"
                && with_nSurf
                ) {
            ERROR_NAMELIST( "nSurf type must be either specular or diffuse" ,
                           LINK_NAMELIST + std::string("#main-variables") );
        }
    PyTools::extract( "specularity", specularity, "Main"   );
    // Quasistatic condition
    PyTools::extract( "quasistatic", quasistatic, "Main"   );
    // Constant external field
    // PyTools::extract( "Ex_const", Ex_const, "Main"   );
    // mfp condition
    PyTools::extract( "with_mfp", with_mfp, "Main"   );
