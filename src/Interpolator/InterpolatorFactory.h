...
// #endif
    ...
    static Interpolator *nSurf( Params &params, Patch *patch, bool vectorization )
    {
        Interpolator *Interp = NULL;
        if(params.with_nSurf) {
            if(params.geometry == "2Dcartesian" ) {
                if(params.nSurf_type=="specular"){
                    Interp = new Interpolator2D0OrderWnSurf_specular( params, patch );
                } else if(params.nSurf_type=="diffuse") {
                    Interp = new Interpolator2D0OrderWnSurf_diffuse( params, patch );
                }
                
            } if(params.geometry == "3Dcartesian" ) { // pending scattering type for 3D
                if(params.nSurf_type=="specular"){
                    Interp = new Interpolator3D0OrderWnSurf_specular( params, patch );
                } else if(params.nSurf_type=="diffuse") {
                    Interp = new Interpolator3D0OrderWnSurf_diffuse( params, patch );
                }
            }
        }
        return Interp;
