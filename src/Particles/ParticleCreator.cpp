     ...
     && species_->file_position_npart_ == 0 ) {
            ...
            for( unsigned int j=0; j<sub_space.box_size_[1]; j++ ) {
                for( unsigned int k=0; k<sub_space.box_size_[2]; k++ ) {
                    ...
                    if( density( i, j, k ) > 0. ) {
                        ...
                        ParticleCreator::createAcc_path(particles_, nPart, iPart);
                        iPart += nPart;
                    }
                }//k
            }//j
        ...
        if( n_new_particles > 0 ) {
            ...
            for( unsigned int ipart = 0; ipart < n_new_particles ; ipart++ ) {
                ...
                // Assign acc_path
                particles_->acc_path( ip ) = 0; //((float) rand()/RAND_MAX)*species_->mfp_; //randomly initilize acc_path so that collisions occur scatteredly in time
                // Assign charge
...
// ---------------------------------------------------------------------------------------------------------------------
//! For all (nPart) particles in a mesh initialize its acc_path
// ---------------------------------------------------------------------------------------------------------------------
void ParticleCreator::createAcc_path( Particles * particles, unsigned int nPart, unsigned int iPart )
{    
    for( unsigned int p= iPart; p<iPart+nPart; p++ ) {
        particles->acc_path( p ) = 0. ;
    }
}
