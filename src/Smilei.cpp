...
int main( int argc, char *argv[] )
{
    ...
    if( params.restart ) 
    {
        if(!params.restart_add_field)
        {
        ...
        } else if( params.restart_add_field ) {
            // smpi.patch_count recomputed in readPatchDistribution
            checkpoint.readPatchDistribution( &smpi, simWindow );
            // allocate patches according to smpi.patch_count
            PatchesFactory::createVector( vecPatches, params, &smpi, openPMD, &radiation_tables_, checkpoint.this_run_start_step+1, simWindow->getNmoved() );
            ...
            // vecPatches.saveExternalFields( params ); //this is causing segmentation fault. saveExternalFields is only used for B-field. Avoid apply external B field
        ...
        vecPatches.initDielectric();
        vecPatches.initnSurf();
        ...
        MESSAGE( 1, "Applying prescribed fields at time t = 0" );
        vecPatches.initializePrescribedFields(time_prim);
            ...
            if( time_dual > params.time_fields_frozen ) {
                #pragma omp parallel shared (time_dual,smpi,params, vecPatches, region, simWindow, checkpoint, itime)
                {
                    ...
                    if(params.quasistatic) {
                        vecPatches.solveMaxwellQuasistatic( params, simWindow, itime, time_dual, timers, &smpi );
                    }
                    else {
        ...
        #pragma omp parallel shared (time_dual,smpi,params, vecPatches, region, simWindow, checkpoint, itime)
        {
            ...
            if( !params.multiple_decomposition ) {
                if( time_dual > params.time_fields_frozen ) {
                    ...
                    if( vecPatches(0)->EMfields->prescribedFields.size() ) {
                        if(!params.constantPrescribedFieldsOnly){
                            #pragma omp single 
                            vecPatches.applyPrescribedFields( time_prim );
                            #pragma omp barrier 
                        }
                        else{
                            vecPatches.applyConstantPrescribedFields(time_prim);
                        }
