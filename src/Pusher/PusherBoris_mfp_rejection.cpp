...
PusherBoris_mfp_rejection::PusherBoris_mfp_rejection( Params &params, Species *species )
    : Pusher( params, species )
{
}

PusherBoris_mfp_rejection::~PusherBoris_mfp_rejection()
{
}
...
void PusherBoris_mfp_rejection::operator()( Particles &particles, SmileiMPI *smpi, int istart, int iend, int ithread, int ipart_buffer_offset , double time)
{
    ...
    for( int ipart=istart ; ipart<iend; ipart++ ) {
        ...
        if(coin < collision_prob){
            ...
            while(temp_mag>1.-1.e-3){
                ...
                temp_mag = sqrt(tempx*tempx+tempy*tempy+tempz*tempz);
            }
