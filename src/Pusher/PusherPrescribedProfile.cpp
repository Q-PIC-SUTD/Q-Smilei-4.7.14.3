...
PusherPrescribedProfile::PusherPrescribedProfile( Params &params, Species *species )
    : Pusher( params, species )
{
}

PusherPrescribedProfile::~PusherPrescribedProfile()
{
}
...
void PusherPrescribedProfile::operator()( Particles &particles, SmileiMPI *smpi, int istart, int iend, int ithread, int ipart_buffer_offset,double time )
{
    ...
    */
    ...
    for( int ipart=istart ; ipart<iend; ipart++ ) {
        ...
        */
        ...
        // Set momentum from oscillation
        momentum_x[ipart] = prescribed_profile->valueAt(time)*cos(prescribed_freq*(time-time_frozen_))*prescribed_amp; // oscillation in x available only; // oscillation in x available only
        momentum_y[ipart] = 0;
        momentum_z[ipart] = 0;
        // MESSAGE(1,"prescribed_freq" << prescribed_freq);
        //MESSAGE(1,"prescribed_amp" << prescribed_amp);
        pxsm = momentum_x[ipart] ;
        pysm = momentum_y[ipart] ;
        pzsm = momentum_z[ipart] ;
