...
PusherBunch::PusherBunch( Params &params, Species *species )
    : Pusher( params, species )
{
}

PusherBunch::~PusherBunch()
{
}

/***********************************************************************
    Lorentz Force -- leap-frog (Drift) scheme
***********************************************************************/

void PusherBunch::operator()( Particles &particles, SmileiMPI *smpi, int istart, int iend, int ithread, int ipart_ref )
{
    ...
    for( int ipart=istart ; ipart<iend; ipart++ ) {
        ...
        dpxCOM += pxsm-momentum[0][ipart];
        dpyCOM += pysm-momentum[1][ipart];
        dpzCOM += pzsm-momentum[2][ipart];
    }
    dpxCOM = dpxCOM/(iend-istart);
    dpyCOM = dpyCOM/(iend-istart);
    dpzCOM = dpzCOM/(iend-istart);
    for( int ipart=istart ; ipart<iend; ipart++ ) {
        charge_over_mass_dts2 = ( double )( charge[ipart] )*one_over_mass_*dts2;

        pxsm = momentum[0][ipart]+dpxCOM;
        pysm = momentum[1][ipart]+dpyCOM;
        pzsm = momentum[2][ipart]+dpzCOM;
