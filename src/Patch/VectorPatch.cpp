...
void VectorPatch::initDielectric()
{
    #pragma omp for schedule(runtime)
    for( unsigned int ipatch=0 ; ipatch<this->size() ; ipatch++ ) {
        ( *this )( ipatch )->EMfields->restartepsilon();
    }
}
void VectorPatch::initnSurf()
{
    #pragma omp for schedule(runtime)
    for( unsigned int ipatch=0 ; ipatch<this->size() ; ipatch++ ) {
        ( *this )( ipatch )->EMfields->restartnSurf();
    }
}
...
void VectorPatch::solveMaxwellQuasistatic( Params &params, SimWindow *simWindow, int itime, double time_dual, Timers &timers, SmileiMPI *smpi )
{
    timers.maxwell.restart();
    ...
    for( unsigned int ipatch=0 ; ipatch<size() ; ipatch++ ) {
        if( params.geometry != "AMcylindrical" ) {
            ...
            if( ( field->name.substr( 0, 2 )=="Jx" ) && (!params.is_pxr) ) {
                ...
                field->allocateDims();
            } else if( ( field->name=="epsilon" ) || (params.is_pxr) ) {
                field->allocateDims();
            } else if( ( field->name=="epsilonx" ) && (!params.is_pxr) ) {
                field->allocateDims(0, false );
            } else if( ( field->name=="epsilony" ) && (!params.is_pxr) ) {
                field->allocateDims(1, false );
            } else if( ( field->name=="epsilonz" ) && (!params.is_pxr) ) {
                field->allocateDims(2, false );
            } else if( ( field->name.substr( 0, 5 )=="nSurf" ) || (params.is_pxr) ) {
                field->allocateDims();            
            }
            //MESSAGE("HNA4");
        } else {
...
{
    // #pragma omp for schedule(static) //Hue 
    for( unsigned int ipatch=0 ; ipatch<size() ; ipatch++ ) {
        patches_[ipatch]->EMfields->applyPrescribedFields( ( *this )( ipatch ), time );
    }
}
void VectorPatch::applyConstantPrescribedFields(double time)
// Prescribed where space and time profile are detangled is applied similar to Antenna
/*
{
    #pragma omp for schedule(static)
    for( unsigned int ipatch=0 ; ipatch<size() ; ipatch++ ) {
        patches_[ipatch]->EMfields->applyConstantPrescribedFields(time);
    }
}
*/
...
{
#ifdef  __DEBUG
    if( nPrescribedField>0 ) {
        #pragma omp single
        TITLE( "Applying prescribed field at time t = " << time );
    }
#endif

    // Loop prescribed fields
    for( unsigned int iPF=0; iPF<nPrescribedField; iPF++ ) {
        
        // if is_it_constant=True, is_it_uniform = False, take profile(r,t) = profile(r,0)
        if( !patches_[0]->EMfields->prescribedFields[iPF].is_it_uniform && patches_[0]->EMfields->prescribedFields[iPF].is_it_constant) {
                #pragma omp for schedule(static)
                for( unsigned int ipatch=0 ; ipatch<size() ; ipatch++ ) {
                    PrescribedField * pf = &( patches_[ipatch]->EMfields->prescribedFields[iPF]);
                    Field *field = patches_[ipatch]->EMfields->allFields[pf->index];
                    field->addedwith(pf->initialField);
                }
                
        
        
        } 
        if( patches_[0]->EMfields->prescribedFields[iPF].is_it_uniform ) {
            // if is_it_constant=True, is_it_uniform = True, take profile(r,t) = profile(r,0)*time_profile(t)
            if( patches_[0]->EMfields->prescribedFields[iPF].is_it_constant ){
                /* //Not needed at the moment
            // Get intensity from antenna of the first patch
                #pragma omp single
                PF_intensity_ = patches_[0]->EMfields->prescribedFields[iPF].time_profile->valueAt( time );
                ...
                for( unsigned int ipatch=0 ; ipatch<size() ; ipatch++ ) {
                    PrescribedField * pf = &( patches_[ipatch]->EMfields->prescribedFields[iPF]);
                    Field *field = patches_[ipatch]->EMfields->allFields[pf->index];
                    field->addedwith(pf->initialField->multiply(PF_intensity_)); // Need to debug this
                    }*/
             // if is_it_constant=False, is_it_uniform = True, take profile(r,t) = profile(r,0)*time_profile(t)    
            } else {
            // Get intensity from antenna of the first patch
                #pragma omp single
                PF_intensity_ = patches_[0]->EMfields->prescribedFields[iPF].time_profile->valueAt( time );
                ...
                for( unsigned int ipatch=0 ; ipatch<size() ; ipatch++ ) {
                    PrescribedField * pf = &( patches_[ipatch]->EMfields->prescribedFields[iPF]);
                    Field *field = patches_[ipatch]->EMfields->allFields[pf->index];
                    field->addedwith_const(PF_intensity_);
                }
            }
...
// For each patch, apply external fields
void VectorPatch::initializePrescribedFields(double time)
{
    // #pragma omp for schedule(static) //Hue 
    // MESSAGE(1,"total no of patches:" << size());
    for( unsigned int ipatch=0 ; ipatch<size() ; ipatch++ ) {
        patches_[ipatch]->EMfields->initializePrescribedFields( ( *this )( ipatch ), time);
        // MESSAGE(1,"done patch #" << ipatch);
    }
}
