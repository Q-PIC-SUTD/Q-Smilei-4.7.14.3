...
public :
    ...
    void solveMaxwellQuasistatic( Params &params, SimWindow *simWindow, int itime, double time_dual,
                       Timers &timers, SmileiMPI *smpi );
    ...
    //! For each patch, apply external time fields
    void applyConstantPrescribedFields(double time);
    //! For each patch, apply external time fields
    void initializePrescribedFields(double time);
    ...
    //! True if any Prescribed Field
    unsigned int nPrescribedField;
