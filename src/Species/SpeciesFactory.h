...
public:
    static Species *create( Params &params, int ispec, Patch *patch )
    {
        ...
        // Extract mean free path from namelist
        double mfp;
        PyTools::extract( "mfp", mfp, "Species", ispec );
        ...
        if( mass > 0. ) {
            ...
            if( pusher == "boris"
                    ...
                    || pusher == "drift"
                    || pusher == "boris_mfp"
                    || pusher == "boris_mfp_normal"
                    || pusher == "boris_mfp_rejection"
                    || pusher == "boris_mfp_2D"
                    || pusher == "boris_2D"
                    || pusher == "prescribed"
                    || pusher == "prescribed_profile"
                    || pusher == "massless"
                    || pusher == "massless_mfp_rejection"
                    || pusher == "vay"
                    || pusher == "higueracary"
        ...
        // Photon species
            ...
            this_species-> pusher_name_ = "norm";  // photon
        ...
        this_species->mfp_ = mfp;
        MESSAGE( "in species mfp" << this_species->mfp_  );
        
        double prescribed_freq;
        PyTools::extract( "prescribed_freq", prescribed_freq, "Species", ispec );
        this_species->prescribed_freq=prescribed_freq;
        double prescribed_amp;
        PyTools::extract( "prescribed_amp", prescribed_amp, "Species", ispec );
        this_species->prescribed_amp=prescribed_amp;
        // MESSAGE("prescribed_freq" << prescribed_freq);
        // MESSAGE("prescribed_amp" << prescribed_amp);
// test prescribed profile
        PyObject * profile_prescribed = nullptr;
        bool has_prescribed_profile = PyTools::extract_pyProfile( "prescribed_profile", profile_prescribed, "Species", ispec );
        if(has_prescribed_profile){
            this_species->prescribed_profile = new Profile( profile_prescribed, 1, "prescribed_profile ", params);
            MESSAGE(2, "> prescribed_profile " << this_species->prescribed_profile->getInfo());

        }
        

        if(pusher=="massless"){
            // this_species-> pusher_name_ = "massless" ;//massless electrons
            double V_const;
            PyTools::extract( "V_const", V_const, "Species", ispec );
            this_species-> V_const = V_const;
        }
        if(pusher=="massless_mfp_rejection"){
            // this_species-> pusher_name_ = "massless" ;//massless electrons
            double V_const;
            PyTools::extract( "V_const", V_const, "Species", ispec );
            this_species-> V_const = V_const;
    ...
    static Species *clone( Species *species, Params &params, Patch *patch )
    {
        ...
        new_species->mfp_                                      = species->mfp_;
        new_species->prescribed_amp                            = species->prescribed_amp;
        new_species->prescribed_freq                           = species->prescribed_freq;
        new_species->prescribed_profile                        = species->prescribed_profile;
        new_species->V_const                                   = species->V_const;
