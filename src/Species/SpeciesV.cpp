    ...
    if( time_dual>time_frozen_ || Ionize ) { // moving particle
        ...
        for( unsigned int ipack = 0 ; ipack < npack_ ; ipack++ ) {
            ...
            ( *Push )( *particles, smpi, particles->first_index[ipack*packsize_],
                       ...
                       ithread, particles->first_index[ipack*packsize_],time_dual );
...
////////////////////////////// new vectorized
    ...
    if( time_dual>time_frozen_ || Ionize ) { // advance particle momentum
        ...
        for( unsigned int ipack = 0 ; ipack < npack_ ; ipack++ ) {
            ...
            timer = MPI_Wtime();
#endif
            ( *Push )( *particles, smpi, particles->first_index[ipack*packsize_], particles->last_index[ipack*packsize_+packsize_-1], ithread, particles->first_index[ipack*packsize_],time_dual );
#ifdef  __DETAILED_TIMERS
