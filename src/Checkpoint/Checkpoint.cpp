...
void Checkpoint::dumpPatch( Patch *patch, Params &params, H5Write &g )
{
    ...
    if (  params.geometry != "AMcylindrical" ) {
        ...
        dumpFieldsPerProc( g, EMfields->epsilon_ );
        dumpFieldsPerProc( g, EMfields->epsilonx_ );
        dumpFieldsPerProc( g, EMfields->epsilony_ );
        dumpFieldsPerProc( g, EMfields->epsilonz_ );
        dumpFieldsPerProc( g, EMfields->nSurfx_ );
        dumpFieldsPerProc( g, EMfields->nSurfy_ );
        dumpFieldsPerProc( g, EMfields->nSurfz_ );
    ...
    for( unsigned int ispec=0 ; ispec<patch->vecSpecies.size() ; ispec++ ) {
        ...
        if( spec->particles->size()>0 ) {
            ...
            s.vect( "Acc_path", spec->particles->Acc_path );//, dump_deflate );
...
void Checkpoint::restartPatch( Patch *patch, Params &params, H5Read &g )
{
    ...
    if ( params.geometry != "AMcylindrical" ) {
        ...
        restartFieldsPerProc( g, EMfields->epsilon_ );
        restartFieldsPerProc( g, EMfields->epsilonx_ );
        restartFieldsPerProc( g, EMfields->epsilony_ );
        restartFieldsPerProc( g, EMfields->epsilonz_ );
        restartFieldsPerProc( g, EMfields->nSurfx_ );
        restartFieldsPerProc( g, EMfields->nSurfy_ );
        restartFieldsPerProc( g, EMfields->nSurfz_ );
    ...
    for( unsigned int ispec=0 ; ispec<patch->vecSpecies.size() ; ispec++ ) {
        ...
        if( partSize>0 ) {
            ...
            s.vect( "Acc_path", spec->particles->Acc_path );
