...
public:
    static ElectroMagn *create( Params &params, DomainDecomposition *domain_decomposition, std::vector<Species *> &vecSpecies,  Patch *patch )
    {
        ...
        params.constantPrescribedFieldsOnly = true;
        for( unsigned int n_extfield = 0; n_extfield < PyTools::nComponents( "PrescribedField" ); n_extfield++ ) {
            ...
            bool is_it_constant;
            PyTools::extract("is_it_constant", is_it_constant, "PrescribedField", n_extfield );
            extField.is_it_constant = is_it_constant;
            bool is_it_uniform;
            PyTools::extract("is_it_uniform", is_it_uniform, "PrescribedField", n_extfield );
            extField.is_it_uniform = is_it_uniform;
            // MESSAGE(extField.is_it_constant);
            // both constant and uniform field can be applied in parallel
            if(!extField.is_it_constant && !extField.is_it_uniform){
                params.constantPrescribedFieldsOnly = false;
            }

            // MESSAGE( 1, "Constant prescribed field only? " << params.constantPrescribedFieldsOnly);
            // Now import the profile
            std::ostringstream name( "" );
            ...
            if(!extField.is_it_uniform){
                ...
                extField.time_profile = NULL;
                MESSAGE( 1, "Make sure profile is a time profile f(x,y,z,t)" )
            } else {
                if( !PyTools::extract_pyProfile( "time_profile", time_profile, "PrescribedField", n_extfield ) ) {
                ERROR( "PrescribedField #"<<n_extfield<<": parameter 'profile' not understood" );
                }
                extField.time_profile = new Profile( time_profile, 1, name.str(), params);
                extField.profile = NULL;
                MESSAGE( 1, "Make sure profile is a time profile f(t)" )
            }
            // MESSAGE(1, "Profile is read, is it uniform? " << extField.is_it_uniform );
            // Find which index the field is in the allFields vector
            extField.index = 1000;
            for( unsigned int ifield=0; ifield<EMfields->allFields.size(); ifield++ ) {
                if( EMfields->allFields[ifield]
                    ...
                    if (params.nDim_field == 1) {
                        ...
                        extField.initialField = new Field1D(EMfields->allFields[ifield]->dims());
                    } else if (params.nDim_field == 2){
                        extField.savedField = new Field2D(EMfields->allFields[ifield]->dims());
                        extField.initialField = new Field2D(EMfields->allFields[ifield]->dims());
                    } else if (params.nDim_field == 3){
                        extField.savedField = new Field3D(EMfields->allFields[ifield]->dims());
                        extField.initialField = new Field3D(EMfields->allFields[ifield]->dims());
                    }
                    extField.savedField->copyFrom(EMfields->allFields[ifield]);
                    extField.initialField->copyFrom(EMfields->allFields[ifield]);
                    extField.savedField->name = EMfields->allFields[ifield]->name;
                    extField.initialField->name = EMfields->allFields[ifield]->name;
                    extField.index =  ifield;
                    break;
                }
            }
            ...
            if( first_creation ) {
                if(extField.is_it_uniform){
                    MESSAGE(1, "Uniform prescribed field " << fieldName << ": " << extField.time_profile->getInfo());
                } else {
    ...
    static ElectroMagn *clone( ElectroMagn *EMfields, Params &params, std::vector<Species *> &vecSpecies,  Patch *patch, unsigned int n_moved )
    {
        ...
        for( unsigned int n_extfield = 0; n_extfield < EMfields->prescribedFields.size(); n_extfield++ ) {
            ...
            newpf.time_profile = EMfields->prescribedFields[n_extfield].time_profile;
            newpf.index   = EMfields->prescribedFields[n_extfield].index;
            newpf.is_it_constant   = EMfields->prescribedFields[n_extfield].is_it_constant;
            newpf.is_it_uniform   = EMfields->prescribedFields[n_extfield].is_it_uniform;
            if (params.nDim_field == 1) {
                newpf.savedField = new Field1D(EMfields->prescribedFields[n_extfield].savedField->dims());
                newpf.initialField = new Field1D(EMfields->prescribedFields[n_extfield].initialField->dims());
            } else if (params.nDim_field == 2){
                newpf.savedField = new Field2D(EMfields->prescribedFields[n_extfield].savedField->dims());
                newpf.initialField = new Field2D(EMfields->prescribedFields[n_extfield].initialField->dims());
            } else if (params.nDim_field == 3){
                newpf.savedField = new Field3D(EMfields->prescribedFields[n_extfield].savedField->dims());
                newpf.initialField = new Field3D(EMfields->prescribedFields[n_extfield].initialField->dims());
            }
            newpf.savedField->copyFrom(EMfields->allFields[newpf.index]);
            newpf.initialField->copyFrom(EMfields->allFields[newpf.index]);
