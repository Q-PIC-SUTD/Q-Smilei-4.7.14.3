...
void ElectroMagn2D::initElectroMagn2DQuantities( Params &params, Patch *patch )
{
    ...
    epsilon_  = new Field2D( dimDual, "epsilon" );
    epsilonx_  = FieldFactory::create( dimPrim, 0, false, "epsilonx", params );
    epsilony_  = FieldFactory::create( dimPrim, 1, false, "epsilony", params );
    epsilonz_  = FieldFactory::create( dimPrim, 2, false, "epsilonz", params );
    nSurfx_  = new Field2D( dimPrim, "nSurfx" );
    nSurfy_  = new Field2D( dimPrim, "nSurfy" );
    nSurfz_  = new Field2D( dimPrim, "nSurfz" );
...
Field * ElectroMagn2D::createField( string fieldname, Params& params )
{
    ...
    else if(fieldname =="epsilon") return new Field2D(dimPrim, fieldname );
    else if(fieldname.substr(0,8)=="epsilonx" ) return FieldFactory::create(dimPrim, 0, false, fieldname, params);
    else if(fieldname.substr(0,8)=="epsilony" ) return FieldFactory::create(dimPrim, 1, false, fieldname, params);
    else if(fieldname.substr(0,8)=="epsilonz" ) return FieldFactory::create(dimPrim, 2, false, fieldname, params);
    else if(fieldname.substr(0,5)=="nSurf") return new Field2D(dimPrim, fieldname );
...
void ElectroMagn2D::applyExternalField( Field *my_field,  Profile *profile, Patch *patch )
{
    ...
    // MESSAGE(1, "is dual x?" << field2D->isDual( 0 ));
    // MESSAGE(1 ,"is dual y?" << field2D->isDual( 1 ));
    
    if ( field2D->name == "epsilon" ) {
        pos[0]      = dx*( ( double )( patch->getCellStartingGlobalIndex( 0 ) )+0.);
        double pos1 = dy*( ( double )( patch->getCellStartingGlobalIndex( 1 ) )+0.);
    }
    vector<Field *> xyz( 2 );
    ...
    if ( field2D->name.substr(0,7) == "epsilon" ) {
        profile->valuesAt( xyz, global_origin, *field2D, 0 );
    } else {
