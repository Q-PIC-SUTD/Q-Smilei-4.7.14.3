...
void ElectroMagn1D::initElectroMagn1DQuantities( Params &params, Patch *patch )
{
    ...
    epsilon_  = new Field1D( dimDual, "epsilon" );
    epsilonx_  = new Field1D( dimPrim, 0, false, "epsilonx" );
    epsilony_  = new Field1D( dimPrim, 1, false, "epsilony" );
    epsilonz_  = new Field1D( dimPrim, 2, false, "epsilonz" );
    nSurfx_  = new Field1D( dimPrim, "nSurfx" );
    nSurfy_  = new Field1D( dimPrim, "nSurfy" );
    nSurfz_  = new Field1D( dimPrim, "nSurfz" );    
...
Field *ElectroMagn1D::createField( string fieldname, Params& params )
{
    if( fieldname.substr( 0, 2 )=="Ex" ) {
        ...
        return new Field1D( dimPrim, 0, false, fieldname );
    } else if( fieldname.substr( 0, 8 )=="epsilonx" ) {
        return new Field1D( dimPrim, 0, false, fieldname );
    } else if( fieldname.substr( 0, 8 )=="epsilony" ) {
        return new Field1D( dimPrim, 1, false, fieldname );
    } else if( fieldname.substr( 0, 8 )=="epsilonz" ) {
        return new Field1D( dimPrim, 2, false, fieldname );
    } else if( fieldname =="epsilon" ) {
        return new Field1D( dimPrim, fieldname );
    } else if( fieldname.substr( 0, 5 )=="nSurf" ) {
        return new Field1D( dimPrim, fieldname );   
    }
...
void ElectroMagn1D::applyExternalField( Field *my_field,  Profile *profile, Patch *patch )
{
    ...
    if ( field1D->name == "epsilon" ) {
        pos = dx * ( ( double )( patch->getCellStartingGlobalIndex( 0 ) )+0. );
    }
    vector<Field *> xyz( 1 );
    ...
    if ( field1D->name.substr(0,7) == "epsilon" ) {
        profile->valuesAt( xyz, global_origin, *field1D, 0 ); // Set value epsilon to profile
    } else {
        profile->valuesAt( xyz, global_origin, *field1D, 1 ); // add external field to existing field
    }
    delete xyz[0];
}
...
void ElectroMagn1D::applyPrescribedField( Field *my_field,  Profile *profile, Patch *patch, double time )
{
    ...
    if(time==0.)
    {
        profile->valuesAt( xyz, global_origin, *field1D, 1, time );
    }
    else{
...
/*
void ElectroMagn1D::applyPrescribedField_initial( Field *my_field,  Field *initial_field, Patch *patch )
{
    Field1D *field1D=static_cast<Field1D *>( my_field );
    Field1D *field1D_add=static_cast<Field1D *>( initial_field );
    
    field1D = field1D+field1D_add;
