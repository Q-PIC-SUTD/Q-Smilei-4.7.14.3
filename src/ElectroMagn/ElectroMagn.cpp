...
void ElectroMagn::finishInitialization( int nspecies, Patch *patch )
{
    ...
    allFields.push_back( epsilon_ );
    allFields.push_back( epsilonx_ );
    allFields.push_back( epsilony_ );
    allFields.push_back( epsilonz_ );
    allFields.push_back( nSurfx_ );
    allFields.push_back( nSurfy_ );
    allFields.push_back( nSurfz_ );
    if( Env_A_abs_ != NULL ) {
...
void ElectroMagn::restartepsilon()
{
    epsilon_ ->put_to( 1. );
    epsilonx_ ->put_to( 1. );
    epsilony_ ->put_to( 1. );
...
void ElectroMagn::initializePrescribedFields( Patch *patch, double time )
{
    for( vector<PrescribedField>::iterator pf=prescribedFields.begin(); pf!=prescribedFields.end(); pf++ ) {
        if( pf->index < allFields.size() ) {
            pf->savedField->copyFrom(allFields[pf->index]);
            if(!pf->is_it_uniform) {
                ...
                allFields[pf->index]->addedwith_const(pf->time_profile->valueAt( time ));
                // MESSAGE(1, "time profile at 0: " << pf->time_profile->valueAt( time ));
            }
            
            pf->initialField->copyFrom(allFields[pf->index]);
        }
        // MESSAGE(1, "done " << pf->index);
    }
    // MESSAGE(1, "done all " );
}
/*
void ElectroMagn::applyConstantPrescribedFields(double time)
{
    for( vector<PrescribedField>::iterator pf=prescribedFields.begin(); pf!=prescribedFields.end(); pf++ ) {
        if( pf->index < allFields.size() ) {
            pf->savedField->copyFrom(allFields[pf->index]);
            if(pf->is_it_constant){
                allFields[pf->index]->addedwith(pf->initialField);
            }
