...
void ElectroMagn3D::initElectroMagn3DQuantities( Params &params, Patch *patch )
{
    ...
    epsilon_  = new Field3D(dimDual, "epsilon" );
    ...
    nSurfx_  = new Field3D( dimPrim, "nSurfx" );
    nSurfy_  = new Field3D( dimPrim, "nSurfy" );
    nSurfz_  = new Field3D( dimPrim, "nSurfz" );    
...
Field *ElectroMagn3D::createField( string fieldname, Params& params )
{
    ...
    else if(fieldname=="epsilon") return new Field3D(dimPrim, fieldname );
    else if(fieldname.substr(0,5)=="nSurf") return new Field3D(dimPrim, fieldname );
...
void ElectroMagn3D::applyExternalField( Field *my_field,  Profile *profile, Patch *patch )
{
    ...
    if ( field3D->name == "epsilonx" )
        dual[0] = true;
    if ( field3D->name == "epsilony" )
        dual[1] = true;
    if ( field3D->name == "epsilonz" )
        dual[2] = true;
    if ( field3D->name == "epsilon" ) {
        dual[0] = false;
        dual[1] = false;
        dual[2] = false;
    } 
    ...
    if ( field3D->name.substr(0,7) == "epsilon" ) {
        profile->valuesAt( xyz, global_origin, *field3D, 0 );
    } else{
