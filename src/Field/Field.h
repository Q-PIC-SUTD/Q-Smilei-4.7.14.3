...
public:
    ...
    inline void __attribute__((always_inline)) plus( double val )
    {
        if( data_ )
            for( unsigned int i=0; i<globalDims_; i++ ) {
                data_[i] += val;
    ...
    inline void __attribute__((always_inline)) addedwith( Field *from_field )
    {
        DEBUGEXEC( if( globalDims_!=from_field->globalDims_ ) ERROR( "Field size do not match "<< name << " " << from_field->name ) );
        for( unsigned int i=0; i< globalDims_; i++ ) {
            ( *this )( i )+=( *from_field )( i );
    ...
    inline void __attribute__((always_inline)) addedwith_const( double val)
    {
        // DEBUGEXEC( if( globalDims_!=from_field->globalDims_ ) ERROR( "Field size do not match "<< name << " " << from_field->name ) );
        for( unsigned int i=0; i< globalDims_; i++ ) {
            ( *this )( i )+= val;
